...
def collect_keys_for_transfer_to_wider_model(module, state_dict, strict):
    ...
    for k, p_target in module.state_dict().items():
        ...
        elif (len(shape_target) == len(shape_source)) and all(
            t >= s for (t, s) in zip(shape_target, shape_source)
        ):
            LOGGER.warning(
                "Transfer to wide model. Found wider parameter: '%s'." "%s vs %s",
                k,
                shape_target,
                shape_source,
            )
...
def transfer_to_wider_model(module, state_dict, strict):
    matching_keys, wider_keys = collect_keys_for_transfer_to_wider_model(
        module, state_dict, strict
    )
...
def transfer_parameters(model, base_model, transfer_config):
    for dst, src in transfer_config.transfer_map.items():
        transfer_state_dict(
            ...
            transfer_config.fuzzy,
            transfer_config.strict,
        )
...
def transfer(model, transfer_config):
    ...
    LOGGER.info("Initiating parameter transfer : '%s'", transfer_config.to_dict())
