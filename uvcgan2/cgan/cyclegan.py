    ...
    def _setup_models(self, config):
        ...
        if self.is_train:
            models["disc_a"] = construct_discriminator(
                config.discriminator, config.data.datasets[0].shape, self.device
            )
            models["disc_b"] = construct_discriminator(
                config.discriminator, config.data.datasets[1].shape, self.device
            )
    ...
    def backward_generators(self):
        ...
        if lambda_idt > 0:
            ...
            self.losses.idt_b = (
                lambda_b
                * lambda_idt
                * self.criterion_idt(self.images.idt_b, self.images.real_b)
            )
            ...
            self.losses.idt_a = (
                lambda_a
                * lambda_idt
                * self.criterion_idt(self.images.idt_a, self.images.real_a)
            )
