...
class Pix2PixModel(ModelBase):
    ...
    def _setup_models(self, config):
        ...
        assert (
            image_shape_a[1:] == image_shape_b[1:]
        ...
        if self.is_train:
            extended_image_shape = (
                image_shape_a[0] + image_shape_b[0],
                *image_shape_a[1:],
            )
    ...
    def _setup_losses(self, config):
        return NamedDict("gen_ab", "gen_ba", "l1_ab", "l1_ba", "disc_a", "disc_b")
    ...
    def __init__(self, savedir, config, is_train, device):
        ...
        assert len(config.data.datasets) == 2, "Pix2Pix expects a pair of datasets"
    ...
    def backward_discriminator_base(self, model, real, fake, preimage):
        ...
        if self.gradient_penalty is not None:
            loss += cal_gradient_penalty(
                model, cond_real, cond_fake, real.device, **self.gradient_penalty
            )[0]
    ...
    def backward_generator_base(self, disc, real, fake, preimage):
        loss_gen = self.criterion_gan(disc(torch.cat([fake, preimage], dim=1)), True)
