...
from uvcgan2.torch.select import select_optimizer, extract_name_kwargs
...
class UVCGAN2(ModelBase):
    ...
    def _construct_batch_head_disc(self, model_config, input_shape):
        disc_body = construct_discriminator(model_config, input_shape, self.device)
    ...
    def _setup_losses(self, config):
        losses = ["gen_ab", "gen_ba", "cycle_a", "cycle_b", "disc_a", "disc_b"]
    ...
    def _setup_optimizers(self, config):
        ...
        optimizers.gen = select_optimizer(
            itertools.chain(
                self.models.gen_ab.parameters(), self.models.gen_ba.parameters()
            ),
        ...
        optimizers.disc = select_optimizer(
            itertools.chain(
                self.models.disc_a.parameters(), self.models.disc_b.parameters()
            ),
    ...
    def __init__(
        ...
        if (lambda_consist > 0) and (consistency is not None):
            self.consist_model = construct_consistency_model(consistency, device)

        assert len(config.data.datasets) == 2, "CycleGAN expects a pair of datasets"
    ...
    def _set_input(self, inputs, domain):
        ...
        if self.images.real_a is not None:
            if self.consist_model is not None:
                self.images.consist_real_a = self.consist_model(self.images.real_a)

        if self.images.real_b is not None:
            if self.consist_model is not None:
                self.images.consist_real_b = self.consist_model(self.images.real_b)
    ...
    def forward_dispatch(self, direction):
        if direction == "ab":
            ...
            self.images.idt_a = self.idt_forward_image(
                self.images.real_a, self.models.gen_ba
            )

        elif direction == "bb":
            self.images.idt_b = self.idt_forward_image(
            ...
            (
                ...
                self.images.real_a, self.models.avg_gen_ab, self.models.avg_gen_ba
            )
            ...
            (
                ...
                self.images.real_b, self.models.avg_gen_ba, self.models.avg_gen_ab
            )
    ...
    def eval_consist_loss(self, consist_real_0, consist_fake_1, lambda_cycle_0):
        return (
            lambda_cycle_0
            * self.lambda_consist
            * self.criterion_consist(consist_fake_1, consist_real_0)
        )
    ...
    def eval_loss_of_idt_forward(self, real_0, idt_0, lambda_cycle_0):
        loss_idt = lambda_cycle_0 * self.lambda_idt * self.criterion_idt(idt_0, real_0)
    ...
    def backward_gen(self, direction):
        if direction == "ab":
            ...
            (self.losses.idt_a, loss) = self.eval_loss_of_idt_forward(
                self.images.real_a, self.images.idt_a, self.lambda_a
            )

        elif direction == "bb":
            (self.losses.idt_b, loss) = self.eval_loss_of_idt_forward(
    ...
    def backward_discriminator_base(self, model, real, fake, queue_real, queue_fake):
        # pylint: disable=too-many-arguments
        loss_gp = None
        ...
        pred_real = queued_forward(model, real, queue_real, update_queue=True)
        loss_real = self.criterion_gan(pred_real, True)

        pred_fake = queued_forward(model, fake, queue_fake, update_queue=True)
    ...
    def backward_discriminators(self):
        ...
        loss_gp_b, self.losses.disc_b = self.backward_discriminator_base(
            self.models.disc_b,
            self.images.real_b,
            fake_b,
            self.queues.real_b,
        ...
        loss_gp_a, self.losses.disc_a = self.backward_discriminator_base(
            self.models.disc_a,
            self.images.real_a,
            fake_a,
            self.queues.real_a,
